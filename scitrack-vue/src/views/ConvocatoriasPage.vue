<template>
  <div>
    ...
    <extra>
      ...
      <v-dialog v-model="editConv" max-width="1200" persistent no-click-animation :style="{ width: 'auto' }">
        <v-card class=" pa-5">
          <v-container>
            <v-row>
              <v-col cols="11">
              ...
              <v-col cols="1">
                <v-btn color="red" :ripple="false" icon @click="editConv = false" style="position: absolute;  top: 8px; right: 8px; z-index: 10;"><v-icon>mdi-close-circle</v-icon></v-btn>
              </v-col>
            </v-row>
          </v-container>
          <v-container fluid style="background-color: #BFD6FF;" class="very-rounded py-5 px-5 mt-4">
            <v-row>
              <v-col cols="12" :md="pdfUrl ? 6 : 12">
                <v-card class="pa-4 " outlined elevation="1" style="background-color: #FFFFFF; border-radius: 10px;">
                  <v-card-text>
                    ...
                    <v-text-field type="text" class="" v-model="currentConv.nombre" placeholder="Ingrese su nombre o nombres..."></v-text-field>

                    <p class="text-h6 font-weight-bold">DESCRIPCIÓN:</p>
                    <v-text-field type="text" class="" v-model="currentConv.descripcion" placeholder="Ingrese la descripcion..."></v-text-field>

                    <p class="text-h6 font-weight-bold">NÚMERO MÁXIMO DE INTEGRANTES:</p>
                    <v-text-field type="number" class="" v-model="currentConv.max_integrantes" placeholder="..."></v-text-field>

                    <p class="text-h6 font-weight-bold">FECHA DE INICIO DE LA CONVOCATORIA:</p>
                    <v-text-field type="date" class="" placeholder="AAAA-MM-DD" v-model="currentConv.fechaInicioRaw"></v-text-field>
                    
                    <p class="text-h6 font-weight-bold">FECHA DE CIERRE DE LA CONVOCATORIA:</p>
                    <v-text-field type="date" class="" placeholder="AAAA-MM-DD" v-model="currentConv.fechaFinRaw"></v-text-field>
                  
                    <div v-if="pdfUrl">
                       <p class="text-h6 font-weight-regular"><b>DOCUMENTO:</b></p>
                          ...
                          <v-col cols="6">
                            <div class="d-flex">
                              <v-btn
                                ...
                                text
                                @click="openFileInNewTab(currentConv.Archivos_idArchivos)"
                              >
                                {{ pdfName }}
                              </v-btn>
                              <v-btn
                                ...
                                text
                                @click="deleteFile(currentConv.Archivos_idArchivos)"
                              >
              ...
              <v-col cols="12" md="6" class="hidden-xs-only">
                <div>
                  <iframe :src="pdfUrl" class="iframe-responsive very-rounded" style="width:100%;"
                    v-if="pdfUrl"></iframe>
                </div>
              </v-col>
            </v-row>
      ...
      <v-dialog v-model="showConv" max-width="1200" persistent no-click-animation :style="{ width: 'auto' }">
        <v-card class=" pa-5">
          <v-container>
            <v-row>
              <v-col cols="11">
                <p class="font-weight-bold text-h4">Información de la Convocatoria</p>
              </v-col>
              <v-col cols="1">
                <v-btn color="red" :ripple="false" icon @click="showConv = false" style="position: absolute;  top: 8px; right: 8px; z-index: 10;"><v-icon>mdi-close-circle</v-icon></v-btn>
              </v-col>
            </v-row>
          </v-container>
          <v-container fluid style="background-color: #BFD6FF;" class="very-rounded py-5 px-5 mt-4">
            <v-row>
              <v-col cols="12" :md="pdfUrl ? 4 : 12">
                <v-card class="pa-4 " outlined elevation="1" style="background-color: #FFFFFF; border-radius: 10px;">
                  <v-card-text>
                    <p class="text-h6 font-weight-regular"><b>NOMBRE:</b> {{ currentConv.nombre }}</p>
                    <p class="text-h6 font-weight-regular"><b>DESCRIPCION:</b> {{ currentConv.descripcion }}</p>
                    <p class="text-h6 font-weight-regular"><b>ESTADO:</b> {{ currentConv.estado }}</p>
                    <p class="text-h6 font-weight-regular"><strong>FECHA DE INICIO:</strong> {{ currentConv.fechaInicio
                    }}</p>
                    <p class="text-h6 font-weight-regular"><strong>FECHA DE CIERRE:</strong> {{ currentConv.fechaFin }}
                    </p>
                    <div v-if="pdfUrl">
                      <p class="text-h6 font-weight-regular"><b>DOCUMENTO:</b></p>
                      <v-btn color="white" style="background-color: #6596FF; overflow: hidden;" text
                        @click="openFileInNewTab(currentConv.Archivos_idArchivos)">{{ pdfName }}</v-btn>
                    </div>
                  </v-card-text>
                </v-card>
              </v-col>
              ...
              <v-card-actions>
                <v-row>
                  <v-col md="4" sm="12" xs="12" lg="2">
                    <v-btn block color="primary" class="bit-more-rounded" depressed
                      @click="openShowConvDIalog(card)">Ver más</v-btn>
                  </v-col>
                  <v-col md="4" sm="12" xs="12" lg="2">
                    <v-btn block color="success" class="bit-more-rounded" depressed @click="openEditDialog(card)">Editar</v-btn>
                  </v-col>
                  <v-col v-if="card.estado != 'Inactiva'" md="4" sm="12" xs="12" lg="2">
                    <v-btn block color="error darken-2" class="bit-more-rounded" depressed
                      @click="openDeleteDialog(card.id)">Eliminar</v-btn>
                  </v-col>
                  <v-col v-if="card.estado == 'Inactiva'" md="4" sm="12" xs="12" lg="2">
                    <v-btn block color="grey " class="bit-more-rounded" depressed
                      @click="alreadyDeleted = true">Eliminar</v-btn>
                  </v-col>
                  <v-col md="4" sm="12" xs="12" lg="2">
                    <v-btn block color="purple darken-3" dark class="bit-more-rounded" @click="openPhases(card)" depressed>Fases</v-btn>
                  </v-col>
                </v-row>
              </v-card-actions>
...
export default {
  ...
  mounted() {
    ...
    Promise.all([
      ...
      fetch("http://localhost:3000/api/convocatoria/").then(res => res.json())
    ])
