<template>
  <div>
    ...
    <div class="pa-7 mt-4 rounded" style="background-color: #ffffff; height: 82.5vmin">
      <v-text-field placeholder="Nombre/ID Participante" label="Buscar" v-model="buscado"
        @input="buscar()"></v-text-field>
      <h1 class="">Participantes</h1>
      <v-row class="px-0 mt-2"
        style=" background-color: #BFD6FF; border-radius: 6px;  border: solid 10px #BFD6FF; overflow-y:scroll; scrollbar-width: none; height: max-content; max-height: 80%;">

        <v-col class="rounded me-2 mx-0 ms-0" cols="8"
          style="margin-left: 1.5vw; background-color: #BFD6FF; height: fit-content; ">
          <v-row v-if="filtrados.length > 1">
            <itemListaUsuario v-for="(filtrado, index) in filtrados" :key="index" :nombre="filtrado.nombre"
              :institucion="filtrado.nombreIns" :convocatoria="filtrado.nombreConvo" :participante="filtrado"
              :vari="vari" :seleccion="elegido" :esUno=false />
          </v-row>
          <v-row style="background-color: #BFD6FF;" v-else>
            <itemListaUsuario v-for="(filtrado, index) in filtrados" :key="index" :nombre="filtrado.nombre"
              :institucion="filtrado.nombreIns" :convocatoria="filtrado.nombreConvo" :participante="filtrado"
              :vari="vari" :seleccion="elegido" :esUno=true />
          </v-row>
                ...
                <div class="rounded" style="max-height: 5.5vw; overflow-y: auto; background-color: #EBF2FF; ">
                  <v-list class="ma-5 my-0 rounded" style="background-color: #EBF2FF; ">
                    <v-radio-group v-model="selectedAreaDeConocimiento" style="margin-top: 0;">
                      <v-radio v-for="AreaDeConocimiento in AreaDeConocimientos" :key="AreaDeConocimiento.nombre"
                        :label="AreaDeConocimiento.nombre" :value="AreaDeConocimiento.nombre"
                        @change="selectedAreaDeConocimiento = AreaDeConocimiento.nombre, onCheckboxChange()"></v-radio>
                    </v-radio-group>
                ...
                <div class="rounded" style="max-height: 5.5vw; overflow-y: auto; background-color: #EBF2FF; ">
                  <v-list class="ma-5 my-0" style="background-color: #EBF2FF; ">
                    <v-radio-group class="" v-model="selectedInstitucion" style="margin-top: 0;">
                      <v-radio class="" v-for="Institucion in Institucions" :key="Institucion.nombre"
                        :label="Institucion.nombre" :value="Institucion.nombre"
                        @change="selectedInstitucion = Institucion.nombre, onCheckboxChange()"></v-radio>
                    </v-radio-group>
      ...
      <transition name="fade">
        ...
        <div v-if="vari.no == 'SI'" class="overlay" @click.self="cerrar">
          <v-card class="pa-8 pt-3 modal-card">
            <v-alert v-if="confirmAlert" type="success" dismissible>
              {{ confirmAlert }}
            </v-alert>
            <v-alert v-if="confirmError" type="error" dismissible>
              {{ confirmError }}
            </v-alert>
            <div style=" font-size: 2.5vmax; font-weight: bold; margin-top: 0vmax; " class="card-header ps-0 mb-3">
              Información</div>
            ...
            <v-row class="rounded py-3 px-2" style="background-color: #BFD6FF;">
              <v-col cols="8">
                <v-card style="width: 100%;">
                  ...
                  <h4 v-if="elegido.selectis.proyecto" class="ps-5 mt-5">Proyecto: {{
                    elegido.selectis.proyecto[0].nombre }}</h4>
                  ...
                  <v-list class="ps-5 mt-0">
                    <v-radio-group class="mt-0">
                      <v-radio v-for="(doc, index) in elegido.archivos" :key="index" :label="doc.nombre" :value="index"
                        @change="abrirArchivo(doc)"></v-radio>
                    </v-radio-group>
                  </v-list>
                  <button @click="generar()" class="ms-5 mt-5 mb-5 rounded pa-2 px-5 aceptar">Generar
                    constancia</button>
                </v-card>
              </v-col>
              <v-col cols="4">
                <iframe :src="pdfUrl" class="iframe-responsive very-rounded" style="width: 100%" v-if="pdfUrl"></iframe>
              </v-col>
            </v-row>
...
export default {
  name: "App",
  components: { itemListaUsuario, LoadingDialog },
  data() {
    return {
