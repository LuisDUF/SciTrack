...
window.onload = async function (){
    const idInvestigador = sessionStorage.getItem('sci:investigador_edit');
    if (idInvestigador == undefined) window.location = "listar.html";
    try {
        startLoad();

        const invesResponse = await fetch(`https://scitrackapi-production.up.railway.app/api/investigador/${idInvestigador}`);
        INVES = await invesResponse.json();

        investigador = INVES[0];
...
function loadData()
{
    ...
    //Fill data
    {
        inputNombre.value = investigador.nombre;
        inputApellidoPaterno.value = investigador.apellidoPaterno;
        inputApellidoMaterno.value = investigador.apellidoMaterno;
        inputCorreo.value = investigador.correo;
        inputNumbers.value = investigador.telefono;
        inputEdad.value = investigador.edad;
        inputDomicilio.value = investigador.domicilio;
        inputCurp.value = investigador.curp;
        inputRfc.value = investigador.rfc;
        inputPassword.value = investigador.contrasenia;
